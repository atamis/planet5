...
public class Game {
	...
	public Game(GameListener listener, Applet p) {
		...
		helpButton = new GameButton(this, p, new Rectangle(w - 63 - 64 - 64, 0, 63, 23), "Help", Fonts.consolas16);
		pauseButton = new GameButton(this, p, new Rectangle(w - 63 - 64, 0, 63, 23), "Pause", Fonts.consolas16);
		quitButton = new ConfirmButton(this, p, new Rectangle(w - 63, 0, 63, 23), "Quit");
		playAgainButton = new GameButton(this, p, new Rectangle((w - 300) / 2, 425, 300, 49), "Play Again", Fonts.consolas32);
		...
		for (int i = 0; buildings.size() == 0 && i < tileHeight; i++) {
			for (int j = 0; buildings.size() == 0 && j < tileWidth; j++) {
				...
				if (good) {
					base = new Building(0, j, i, gameMillis);
					buildings.add(base);
	...
	public void update() {
		...
		if (placingBuilding != -1 && curEnergy < BuildingStats.costs[placingBuilding]) {
			placingBuilding = -1;
		}
	}
		...
		for (Building building : buildings) {
			if (building.powered) {
				...
				if (building == base) {
					...
					curEnergy += elapsedMillis * BuildingStats.gen[building.type];
				}
		...
		for (int i = 0; i < tileHeight; i++) {
			for (int j = 0; j < tileWidth; j++) {
				...
				if (!tiles[i][j].wall && tiles[i][j].building == null &&
					...
					Enemy enemy = new Enemy(j * TILE_SIZE, i * TILE_SIZE, type, this, p);
					enemies.add(enemy);
				}
			}
		}
	...
	private void updateEnemies(int elapsedMillis) {
		for (Enemy enemy : enemies) {
			...
			Rectangle inflated = new Rectangle((int) enemy.bounds.x, (int) enemy.bounds.y, enemy.ENEMY_SIZE, enemy.ENEMY_SIZE);
			inflated.grow(1, 1);
	...
	public void placeBuilding() {
		...
		if (isPlacingBuilding() && canPlaceBuilding(x, y, w, h)) {
			Building placedBuilding = new Building(buildingType, x, y, gameMillis);
	...
	public void mousePressed(int x, int y, int mouseButton) {
		...
		for (int i = 0; i < 6; i++) {
			...
			if (mouseButton == MouseEvent.BUTTON1 && x >= boxX && y >= 1 &&
					curEnergy >= BuildingStats.costs[i + 1] && curEnergy >= BuildingStats.costs[i + 1] &&
		...
		if (x >= BAR_HEIGHT) {
			...
			int row = (y + mapY - BAR_HEIGHT) / TILE_SIZE; // TODO: refactor, update every tick?
